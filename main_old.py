...
def start(message):
    for user in data["users"]:
        if user["id"] == message.from_user.id:
            ...
            flag = False
    if flag == False:
        data["users"].append({"id": message.from_user.id,"bot_id": data["users"][-1]["bot_id"] + 1, "username": message.from_user.username,"blacklist":'',"start_parsing": False,"number_of_page": 0,"tag": "","favourite": '',"stop": False,"current_image": 0,"last_image": 0,"now": 0})
...
def search(message):
    id = message.text.replace('/search','')
    try:
        url = 'https://rule34.xxx/index.php?page=post&s=view&id={0}'.format(id)
        soup = BeautifulSoup(requests.get(url).text,'html.parser')
        link = soup.find('img',{'id':'image'}).get('src')
        if link[-len(str(id))-3] == 'g':
            bot.send_video(message.from_user.id,link,caption=id)
        else:
            bot.send_photo(message.from_user.id,link,caption=id)
    except:
        try:
            link = soup.find('source',{'type':'video/mp4'}).get('src')
...
def tags(message):
    id = message.text.replace('/tags','')
    try:
        url = 'https://rule34.xxx/index.php?page=post&s=view&id={0}'.format(id)
        soup = BeautifulSoup(requests.get(url).text,'html.parser')
        pics_tags = soup.find('img',{'id':'image'}).get('alt').replace(' ','     ')
        bot.send_message(message.from_user.id,pics_tags.replace(',',''))
    except:
        try:
            video_tags = soup.find('meta',{'name':'keywords'}).get('content').replace(' ','     ')
            bot.send_message(message.from_user.id,video_tags)
        except Exception as error:
        ...
        if user["id"] == message.from_user.id:
            if message.chat.type == 'private':
                if message.text == 'Blacklist':
                    ...
                    favorite_split = user["favourite"].split()
                    for item in favorite_split:
                        def favourite_search():
                            url = 'https://rule34.xxx/index.php?page=post&s=view&id={0}'.format(item)
                            soup = BeautifulSoup(requests.get(url).text,'html.parser')
                            try:
                                link = soup.find('img',{'id':'image'}).get('src')
                                if link[-len(str(item))-4] == 'g':
                                    bot.send_video(user["id"],link,caption=item)
                                else:
                                    bot.send_photo(user["id"],link,caption=item)
                            except:
                                try:
                                    link = soup.find('source',{'type':'video/mp4'}).get('src')
                                    bot.send_video(message.chat.id,link,caption=item)
                                except Exception as error:
                                    print('{0} in favourite_search: {1}'.format(user["username"],error))
                        favourite_search()
                    bot.send_message(message.chat.id,'избранное {0}'.format(user["username"]))
